...
def generate_document(levels = 1, load_graph = None, write_graph = False):
    ...
    concept_graph = kg.graph_inference(concept_graph)
    
    # Generate a visualization of the core graph nodes
    kg_viz.create_graph(concept_graph, variable_input)
...
def display_svo_categorizations(graph, user_input):
    det_category = graph[user_input.lower()]['detSVOCategory']
...
def display_related_terms(graph, user_input):
    if 'modified_terms' in graph[user_input.lower()].keys():
        modified_terms = graph[user_input.lower()]['modified_terms']
        if modified_terms != []:
    ...
    if 'term_aspects' in graph[user_input.lower()].keys():
        term_aspects = graph[user_input.lower()]['term_aspects']
        if term_aspects != []:
...
def display_svo_variables(graph, user_input):
    ...
    if 'hasSVOVar' in graph[user_input.lower()]:
        variables = graph[user_input.lower()]['hasSVOVar']
